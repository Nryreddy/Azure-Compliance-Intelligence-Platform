...
compliance audit workflow. its a master switch that:
1. Sets up the audit request
2. Runs the AI workflow
3. Displays the final compliance report
"""

import uuid      # Generates unique IDs (for session tracking numbers)
import json      # Handles JSON data formatting (converts Python dicts to readable text)
import logging   # Records what happens during execution
from pprint import pprint  # Pretty-prints data structures


from dotenv import load_dotenv
load_dotenv(override=True)  #.env values take priority over system variables
...
logging.basicConfig(
    ...
    # Example: "2026-01-15 10:30:45 - AI-compliance-auditor - INFO - Starting audit"
)
logger = logging.getLogger("AI-compliance-auditor")  # Creates a named logger for this module
